 ...
 *
 * Each instance is bound to a single [recipient] and a single request – never shared
 * across concurrent coroutines.
 */
/**
 * Dedicated thread pool for tool HTTP calls.
 * Sized to 2× available processors, isolated from [Dispatchers.IO] so tool
 * calls cannot starve webhook or agent coroutines on the shared IO pool.
 */
private val toolDispatcher = Executors
...
class WhatsAppTools(
    ...
    private fun <T> io(block: suspend () -> T): T =
        runBlocking(toolDispatcher) { block() }

    @Tool
    @LLMDescription("Send a plain text reply to the current WhatsApp user")
    fun sendTextReply(
        @LLMDescription("The text message body to send")
        message: String,
    ): String {
    ...
    fun sendImage(
        @LLMDescription("Publicly accessible URL of the image to send")
        imageUrl: String,
        @LLMDescription("Optional caption for the image")
        caption: String = "",
    ): String {
    ...
    fun sendDocument(
        @LLMDescription("Publicly accessible URL of the document to send")
        documentUrl: String,
        @LLMDescription("Display filename shown to the recipient")
        filename: String,
        @LLMDescription("Optional caption for the document")
        caption: String = "",
    ): String {
    ...
    fun sendReaction(
        @LLMDescription("The WhatsApp message ID to react to")
        messageId: String,
        @LLMDescription("The emoji character to react with, e.g. 👍")
        emoji: String,
    ): String {
    ...
    fun sendButtons(
        @LLMDescription("The body text of the message shown above the buttons")
        message: String,
        ...
        return try {
            val parsed = buttons.split(",").map { pair ->
