 ...
 *   POST {baseUrl}/meta/whatsapp/{phoneNumberId}/messages
 *
 * Media is uploaded to:
 *   POST {baseUrl}/meta/whatsapp/{phoneNumberId}/media
 */
class KapsoClient(private val config: KapsoConfig) {
    ...
    private val messagesUrl get() =
        "${config.baseUrl}/meta/whatsapp/${config.phoneNumberId}/messages"

    private val mediaUrl get() =
        "${config.baseUrl}/meta/whatsapp/${config.phoneNumberId}/media"
    ...
    suspend fun sendText(to: String, body: String): SendMessageResponse {
        ...
        return http.post(messagesUrl) {
            headers { bearerAuth(config.apiKey) }
    ...
    suspend fun sendImage(to: String, url: String? = null, mediaId: String? = null, caption: String? = null): SendMessageResponse {
        require(url != null || mediaId != null) { "Either url or mediaId must be provided for image" }
    ...
    suspend fun sendVideo(to: String, url: String? = null, mediaId: String? = null, caption: String? = null): SendMessageResponse {
        require(url != null || mediaId != null) { "Either url or mediaId must be provided for video" }
        val body = MediaBody(id = mediaId, link = url, caption = caption)
        return http.post(messagesUrl) {
            headers { bearerAuth(config.apiKey) }
            contentType(ContentType.Application.Json)
    ...
    suspend fun sendAudio(to: String, url: String? = null, mediaId: String? = null): SendMessageResponse {
        require(url != null || mediaId != null) { "Either url or mediaId must be provided for audio" }
    ...
    suspend fun sendDocument(
        ...
        require(url != null || mediaId != null) { "Either url or mediaId must be provided for document" }
     ...
     */
    suspend fun sendButtons(to: String, body: String, buttons: List<Pair<String, String>>): SendMessageResponse {
        require(buttons.size in 1..3) { "WhatsApp supports 1–3 quick reply buttons" }
        val payload = SendInteractiveRequest(
            to = to,
            interactive = InteractivePayload(
                body = InteractiveBody(body),
                action = InteractiveAction(
                    buttons = buttons.map { (id, title) ->
                        InteractiveButton(reply = ButtonReply(id, title))
                    }
                )
            )
        )
    ...
    /**
     * Upload raw bytes to Kapso/Meta and return the media ID.
     * Use the returned ID in subsequent [sendImage]/[sendDocument] etc. calls.
     */
    suspend fun uploadMedia(fileBytes: ByteArray, mimeType: String, filename: String): String {
        ...
        val response: UploadMediaResponse = http.post(mediaUrl) {
            headers { bearerAuth(config.apiKey) }
            setBody(
    ...
    suspend fun getMediaUrl(mediaId: String): String? {
        val response: MediaUrlResponse = http.get("${config.baseUrl}/meta/whatsapp/media/$mediaId") {
            headers { bearerAuth(config.apiKey) }
        }.body()
        return response.url
    }
